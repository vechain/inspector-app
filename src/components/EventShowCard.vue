    ...
    <div>
      ...
      >
        <div class="level-left">
          <span class="is-family-secondary display-6 has-text-black-ter has-text-weight-semibold">
            <slot name="title"></slot>
          </span>
          <span style="margin-left: 20px" class="is-family-secondary display-6">{{item.meta.blockTimestamp * 1000 | datetime}}</span>
        </div>
        <div class="level-right">
          <a
            class="s-family-monospace display-6"
            :href="`https://insight.vecha.in/#/tx/${item.meta.txID}`"
      ...
      >
        <b-tab-item label="Raw">
          <div>
            <b-field custom-class="my-label" label-position="inside" horizontal label="Topics">
              <ul>
                <li
            ...
            <b-field custom-class="my-label" label-position="on-border" horizontal label="Data">
              <p
        ...
        <b-tab-item label="Decoded">
          <b-table :data="data">
            <template slot-scope="props">
              <b-table-column label="#" field="id">{{props.row.id}}</b-table-column>
              <b-table-column label="Name" field="name">{{props.row.name}}</b-table-column>
              <b-table-column label="Type" field="type">
                {{props.row.type}}
                <sup class="is-size-7" v-if="props.row.value">indexed</sup>
              </b-table-column>
